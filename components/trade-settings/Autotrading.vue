...
<template>
  <div class="trade-trading">
    <div class="trade-trading__header">
      {{ $t('Autotrading') }}
      <div class="line" />
    </div>

    <div class="trade-trading__panel">
      <div class="left">
        <div v-if="!isLoading">
          <button class="play" :class="{ active: isAutoTrade === 1 }" @click="handleTradeToggle(true, false)">
          ...
          <button class="pause" :class="{ active: isStopTrade === 1 }" @click="handleTradeToggle(false, true)">
      ...
      <div class="right">
        <span>{{ $t('Orders count BUY/SELL') }} {{ allOrdersCount ? allOrdersCount : 0 }}</span>
      </div>
    </div>
  </div>
</template>
...
.trade-trading__header .line {
  width: 100%;
  border-bottom: 1px dashed rgba(255, 255, 255, 0.3);
}

.trade-trading__panel {
  margin-top: 10px;
...
.trade-trading__panel .left button {
...
.trade-trading__panel .left button.active.play {
  svg path {
    stroke: #B2E500;
  }
}

.trade-trading__panel .left button.active.pause {
  svg path {
    stroke: #F89900;
  }
}

.trade-trading__panel .left button.active.stop {
  svg path {
    stroke: #BE5D5D;
  }
}

.trade-trading__panel .right button {
  outline: none;
  background-color: transparent;
  border: none;
  color: #BE5D5D;

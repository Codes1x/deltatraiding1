...
const changeLang = async (lang: "en" | "ru") => {

    try {
        const payload = {
            language: lang
        }
        await profileStore.setLanguage(payload)
        setLocale(lang) // Меняем локаль через i18n
    } catch (error) {
...
<template>
    <div class="lang-switcher">
        <NuxtLink v-if="locale === 'en'" :to="switchLocalePath('ru')" @click="changeLang('ru')">
            <span class="flag icon">
                <svg width="21" height="16" viewBox="0 0 21 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g clip-path="url(#clip0_335_10)">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M0 0H21V16H0V0Z" fill="white" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M0 5.33334H21V16H0V5.33334Z" fill="#0039A6" />
        ...
        <NuxtLink v-else :to="switchLocalePath('en')" @click="changeLang('en')">
            <span class="flag icon">
                <svg width="32" height="22" viewBox="0 0 512 342" xmlns="http://www.w3.org/2000/svg">
                    <g fill="none">

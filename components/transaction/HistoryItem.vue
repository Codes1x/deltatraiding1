...
<template>
  <div class="transaction__history-item" :class="{ danger: transaction.amount.includes('-') }">
    <div class="left">
      <div class="image">
      ...
      <div class="text">
        ...
        <div v-if="transaction.transaction_type === 'purchase'">
          {{ $t('Payment') }}
        </div>
        <div v-else-if="transaction.transaction_type === 'replenishment'">
          {{ $t('Replenishment') }}
        </div>
        <div v-else-if="transaction.transaction_type === 'transfer'">
          {{ $t('transfer') }}
        </div>
        <div v-else>
          {{ transaction.amount.includes('-') ? $t('Sended') : $t('Income') }}
        </div>
        <span>{{ transaction.description.includes('fee') ? 'External replenishment' : transaction.description }}</span>
      </div>
    </div>

    <div class="right">
      <span class="amount">{{
        parseFloat(transaction.amount.replace(/^[-+]/, '')).toFixed(2) }}</span>

      <span class="date">{{ formatDate(transaction.created_at.toString(), locale) }}</span>
    </div>
  </div>
</template>
